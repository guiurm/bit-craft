...
import DOMEventManager from '@/@core/dom-manager/DOMEventManager';
...
const domManager = new DOMEventManager(document.body);
let offsetX = 0;
let offsetY = 0;
...
onMounted(() => {
    if (props.targetContainer && draggingItemClone.value) domManager.changeTarget(dragContainer.value as HTMLElement);
});
</script>

<template>
    <div ref="dragContainer" class="flex flex-col gap-1 overflow-hidden">
        <template v-for="item in itemsRef" :key="item.id">
            <d-i
                :model-value="item"
                @update:model-value="n => updateItem(item, n)"
